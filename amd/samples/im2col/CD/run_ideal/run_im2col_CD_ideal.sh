...
cd /root/mgpusim_home/mgpusim/amd/samples/im2col/CD/run_ideal
...
mv akita_sim_*.sqlite3 /root/mgpusim_home/results/CD/rawdata/sql/im2col_ideal.sqlite3 2>/dev/null

# motivation 경로에도 복사 (summarize.py 호환)
cp /root/mgpusim_home/results/CD/rawdata/sql/im2col_ideal.sqlite3 /root/mgpusim_home/results/motivation/rawdata/sql/im2col_motivation.sqlite3 2>/dev/null

# Coalescability CSV 수집
for csv_file in motivation_coalescability_GPU*.csv motivation_cumulative_GPU*.csv; do
    [ -f "$csv_file" ] && mv "$csv_file" "/root/mgpusim_home/results/motivation/rawdata/csv/im2col_$csv_file"
done
